          ...
          <tbody>
            {vouchers.map((v) => (
              <tr key={v.id} className="border-t border-slate-100" data-testid={`voucher-row-${v.id}`}>
                ...
                <td className="p-3"><a href={v.pdf_url || "#"} target="_blank" rel="noreferrer" className="text-[#5A8F1E] hover:underline text-xs">{v.pdf_url ? "Ver PDF" : "—"}</a></td>
          ...
          <tbody>
            {orders.map((o) => (
              <tr key={o.order_no} className="border-t border-slate-100" data-testid={`order-row-${o.order_no}`}>
                ...
                <td className="p-3 text-xs">{o.delivery?.method === "hand_delivery" ? "Em mão" : "Levantamento"}</td>
